...
err_strongX1_array = np.zeros((dW_realizations, len(dt_list)))
err_strongX2_array = np.zeros((dW_realizations, len(dt_list)))
err_weakX1_array = np.zeros((dW_realizations, len(dt_list)))
err_weakX2_array = np.zeros((dW_realizations, len(dt_list)))
...
for i in range(dW_realizations):
    ...
    for j in range(len(dt_list)):
        ...
        err_strongX1_array[i][j] = np.abs(X_exact[0][-1]-X_approx[0][-1])
        err_strongX2_array[i][j] = np.abs(X_exact[1][-1]-X_approx[1][-1])
        err_weakX1_array[i][j] = (X_exact[0][-1]-X_approx[0][-1])
        err_weakX2_array[i][j] = (X_exact[1][-1]-X_approx[1][-1])
    
err_strongX1_list = np.mean(err_strongX1_array, axis=0)
err_strongX2_list = np.mean(err_strongX2_array, axis=0)
err_weakX1_list = np.abs(np.mean(err_weakX1_array, axis=0))
err_weakX2_list = np.abs(np.mean(err_weakX2_array, axis=0))
    

plt.loglog(dt_list, err_strongX1_list)
plt.loglog(dt_list, err_strongX2_list)
plt.xlabel("dt")
...
plt.loglog(dt_list, err_weakX1_list)
plt.loglog(dt_list, err_weakX2_list)
plt.xlabel("dt")
